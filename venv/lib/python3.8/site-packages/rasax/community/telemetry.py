...
def track_project_status(session: Optional[Session] = None) -> None:
    ...
    try:
        cm = common_utils.nullcontext(session) if session else db_utils.session_scope()
        with cm as db_session:
...
@ensure_telemetry_initialized
def track_training_failed(rasa_open_source: str) -> None:
    """Tracks an event when a model training fails.

    Args:
        rasa_open_source: Rasa Open Source version.
    """
    track(MODEL_TRAINING_FAILED_EVENT, {"rasa_open_source": rasa_open_source})
