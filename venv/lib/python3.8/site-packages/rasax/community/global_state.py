from typing import NamedTuple, Text, List, TYPE_CHECKING
...
class GlobalState(NamedTuple):
    ...
    telemetry_write_key: Text

    jobs_queue: mp.Queue

    git_global_operation_lock: "Lock"
    is_target_branch_ahead: "_Value"

    is_local_mode: bool
    project_directory: "_Array"

    were_models_discovered: "_Value"

    websocket_queues: List[mp.Queue]

    timestamp_of_oldest_pending_change: "_Value"
    timestamp_of_latest_pending_change: "_Value"


def close_global_resources() -> None:
    """Close global resources to free them up."""
    import rasax.community.utils.common

    state = get_global_state()
    rasax.community.utils.common.mp_context().Manager().shutdown()

    state.telemetry_queue.close()
    state.jobs_queue.close()
    for queue in state.websocket_queues:
        queue.close()
...
def initialize_global_state(
    ...
    from rasax.community.services import (
    ...
    tags_service.initialize_global_state(mp_context)
