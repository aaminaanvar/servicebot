...
def blueprint() -> Blueprint:
    """Declare endpoints related to conversations.

    Returns:
        Blueprint with endpoints related to conversations.
    """
    ...
    def _get_clients(
        ...
        if not role_service.user_has_permission(
            user, "conversations.view.metadata.list"
        ):
    ...
    async def put_conversation_review_status(
        ...
        try:
            previous_review_status = EventService.from_request(
                request
    ...
    async def delete_conversation_by_id(
        ...
        """
        try:
            EventService.from_request(request).delete_conversation_by_id(
                conversation_id
            )
    ...
    # We cannot have a "/tracker" endpoint since it's blocked by some adblockers.
    ...
    def _has_access_to_conversation(
        ...
        if role_service.user_has_permission(
            user, "conversations.view.conversationEvents.list"
        ):
            return True
    ...
    async def get_conversation_tracker_impl(
        ...
        if not _has_access_to_conversation(
            event_service, role_service, conversation_id, user
        ):
            return common_utils.error(
                HTTPStatus.UNAUTHORIZED, "NoPermission", "Access denied"
            )

        test_story = common_utils.bool_arg(request, "test_story", False)
        until_time = common_utils.float_arg(request, "until", None)
        since_time = common_utils.float_arg(request, "since", None)
        rasa_environment_query = common_utils.deployment_environment_from_request(
            request, None
        )
        ...
        if output_format != data.FileFormat.JSON:
            _events = events.deserialise_events(tracker["events"])
            story = Story.from_events(_events)
            story.story_name = f"Story from Conversation ID {conversation_id}"

            if output_format == data.FileFormat.MARKDOWN:
                # Note: e2e is the old name for test stories
                exported = story.as_story_string(flat=True, e2e=test_story)
                ...
                for step in story.story_steps:
                    step.block_name = story.story_name

                return response.text(
                    writer.dumps(story.story_steps, is_test_story=test_story),
                    content_type="application/x-yaml",
                    headers={
                        "Content-Disposition": f"attachment;filename={conversation_id}-story.yml"
                    },
                )
        ...
        return response.json(
            ...
            headers={
                "Content-Disposition": f"attachment;filename={conversation_id}-dump.json"
            },
        )
    ...
    async def unique_actions(request):
        actions = EventService.from_request(request).get_unique_actions()
    ...
    async def unique_intents(request):
        intents = EventService.from_request(request).get_unique_intents()
    ...
    async def unique_entities(request):
        entities = EventService.from_request(request).get_unique_entities()
    ...
    async def unique_policies(request):
        policies = EventService.from_request(request).get_unique_policies()
    ...
    async def post_event(request, conversation_id, user):
        _event = request.json
        try:
            _ = await StackService.from_request(
                request,
                rasa_x_config.project_name,
                constants.RASA_PRODUCTION_ENVIRONMENT,
    ...
    async def execute_action(request, conversation_id):
        ...
        try:
            result = await StackService.from_request(
                request,
                rasa_x_config.project_name,
                constants.RASA_PRODUCTION_ENVIRONMENT,
    ...
    async def update_conversation_evaluations(request):
        ...
        for _id in conversation_ids:
            ...
            try:
                content = await StackService.from_request(
                    request,
                    rasa_x_config.project_name,
                ...
                return common_utils.error(
                    ...
                    f"Failed to store evaluation for conversation_id '{_id}'."
    ...
    async def get_evaluations(request):
        """Fetches a list of Core (Stories) Evaluations."""
        event_service = EventService.from_request(request)
        conversation_ids = event_service.conversation_ids()
        evaluations = []
        for _id in conversation_ids:
    ...
    async def get_evaluation(request, conversation_id):
        evaluation = EventService.from_request(request).get_evaluation(conversation_id)
    ...
    async def delete_evaluation(request, conversation_id):
        try:
            EventService.from_request(request).delete_evaluation(conversation_id)
            return response.text("", HTTPStatus.NO_CONTENT)
        except ValueError as e:
            return common_utils.error(
                HTTPStatus.NOT_FOUND,
                "EvaluationNotFound",
                f"Failed to delete evaluation for conversation_id '{conversation_id}'.",
                details=e,
            )
    ...
    async def put_evaluation(request, conversation_id):
        ...
        try:
            content = await StackService.from_request(
                request, rasa_x_config.project_name, constants.RASA_WORKER_ENVIRONMENT
            ).evaluate_story(story)
    ...
    async def predict_next_action(request, conversation_id):
        ...
        try:
            ...
            return common_utils.error(
                ...
                f"Failed to predict the next action for conversation_id"
                f" '{conversation_id}'.",
                details=e,
            )
    ...
    async def flag_message(
        ...
        EventService.from_request(request).add_flagged_message(
    ...
    async def delete_flag_from_conversation(
        ...
        EventService.from_request(request).delete_flagged_message(
            conversation_id, message_timestamp
        )
    ...
    async def correct_message(
        ...
        try:
            ...
            EventService.from_request(request).correct_message(
    ...
    async def delete_message_correction(
        ...
        try:
            ...
            EventService.from_request(request).delete_message_correction(
    ...
    async def chat_to_bot(request, conversation_id=None):
        ...
        service = StackService.from_request(
            request, rasa_x_config.project_name, constants.DEFAULT_RASA_ENVIRONMENT
        )
        if not service:
            return common_utils.error(
                HTTPStatus.NOT_FOUND,
    ...
    async def issue_signed_jwt(request):
        ...
        chat_token = DomainService.from_request(request).get_token()
        if not chat_token:
            return common_utils.error(
                HTTPStatus.NOT_FOUND,
                "ChatTokenNotFound",
        ...
        if DomainService.from_request(request).has_token_expired(
            id_object["chat_token"]
        ):
            return common_utils.error(
