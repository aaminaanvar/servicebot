...
def blueprint():
    ...
    async def suggestions(request: Request, project_id: Text) -> HTTPResponse:
        ...
        workspace_id = common_utils.int_arg(request, "workspace_id", None)
    ...
    @logs_endpoints.route("/projects/<project_id>/logs", methods=["PATCH"])
    @rasa_x_scoped("examples.update")  # TODO: Revisit this
    @inject_rasa_x_user()
    async def update_logs(
        request: Request, project_id: Text, user: Dict
    ) -> HTTPResponse:
        """Updates existing message logs."""
        logs_data = request.json.get("logs")
        logs_service = LogsService.from_request(request)
        updated_logs = logs_service.update_logs(logs_data)

        if updated_logs is None:
            return common_utils.error(
                HTTPStatus.BAD_REQUEST,
                "ExampleUpdateFailed",
        ...
        if updated_logs:
            websocket_service.send_message(
                websocket_service.Message(
                    websocket_service.MessageTopic.NLU,
                    name="message_log_updated",
                    data={"drafts": updated_logs},
                )
            )

        return response.json(updated_logs)
