        ...
        async def decorated_function(request, *args, **kwargs):
            ...
            with instant_config(instance, request=request, **kw):
                ...
                if allow_rasa_x_token:
                    rasa_x_token = common_utils.default_arg(request, "token", "")
                    if secrets.compare_digest(rasa_x_token, rasa_x_config.rasa_x_token):
    ...
    def decorator(f):
        ...
        async def decorated_function(
            ...
            try:
                ...
                logger.error(
                    f"Request of type '{request.method}' for URL "
                    f"'{request.raw_url.decode(DEFAULT_ENCODING)}' had a payload which "
                    f"didn't match schema {schema}. Details:\n{e}"
                )
                return common_utils.error(
                    HTTPStatus.BAD_REQUEST,
                    "WrongSchema",
