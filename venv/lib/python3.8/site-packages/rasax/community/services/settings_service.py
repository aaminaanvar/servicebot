...
class SettingsService(DbService):
    ...
    def inject_environments_config_from_file(self, project_id: Text, filename: Text):
        ...
        try:
            ...
            if current_config:
                logger.warning(
                    f"Rasa X will continue to use your current configuration."
                    f"Your current configuration is:\n{json.dumps(current_config, indent=2)}"
                )
            else:
                raise RasaXEnvironmentError(
                    "Could not get valid environments for server startup."
                )
    ...
    def inspect_environments_config(env_config: Dict[Text, Any]) -> None:
        ...
        Args:
            env_config: The environments config to inspect.
        ...
        if not rasa_config:
            raise RasaXEnvironmentError("Environment config needs to have key `rasa`.")

        if any(key not in rasa_config for key in constants.ENVIRONMENTS_MANDATORY_KEYS):
            raise RasaXEnvironmentError(
        ...
        if community_unallowed_keys:
            ...
            for key in community_unallowed_keys:
                del rasa_config[key]

    def save_environments_config(
        self, project_id: Text, environments_config: Dict[Text, Any]
    ) -> Dict[Text, Dict[Text, Dict[Text, Text]]]:
        """Save the provided environments configuration to the project.

        Args:
            project_id: The project to apply the enviornments to.
            environments_config: The environments configuration to save to the project

        Returns:
            The environments config stored under the `project_id` after saving.
        """
