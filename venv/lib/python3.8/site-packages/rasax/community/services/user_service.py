...
import random
import time
from typing import Optional, Text, Dict, List, Any, Union, TYPE_CHECKING
...
class UserService(DbService):
    ...
    def create_user(
        ...
        if not common_utils.is_enterprise_installed() and username not in (
    ...
    def change_password(self, fields: Dict[Text, Text]) -> Optional[Dict]:
        ...
        self._update_community_user_password(
            fields[constants.USERNAME_KEY], fields["new_password"]
        )
        return user.as_dict()
    ...
    def update_user(self, username: Text, values: Dict[Text, Any]) -> None:
        ...
        Only the `data` and `default_workspace_id` properties can be updated at
        the moment.
        ...
        if "default_workspace_id" in values:
            user.default_workspace_id = values["default_workspace_id"]

    def admin_change_password(self, username: Text, password: Text) -> Dict[Text, Any]:
    ...
    def _update_community_user_password(self, username: Text, password: Text) -> None:
        if username == constants.COMMUNITY_USERNAME:
            from rasax.community.services.settings_service import SettingsService

            SettingsService(self.session).save_community_user_password(password)
    ...
    def assign_project_to_user(self, user: Dict, project_id: Text) -> None:
        ...
        owner = self._fetch_user(user.get(constants.USERNAME_KEY))
        if not user:
            return
