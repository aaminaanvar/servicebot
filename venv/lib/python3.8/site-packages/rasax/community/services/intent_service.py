...
class IntentService(DbService):
    ...
    def _add_suggestion_to_intents(
        ...
        for i in intents:
            suggestions, _ = logs_service.fetch_logs(
                intent_query=i[INTENT_NAME_KEY],
                fields_query=[("hash", True)],
    ...
    @staticmethod
    def from_request(request: Request, **kwargs) -> "IntentService":
        """Creates an `IntentService` from an incoming HTTP request's DB session.
        ...
        Returns:
            `IntentService` instance with a connection to the DB.
        """
        return IntentService(request.ctx.db_session)
