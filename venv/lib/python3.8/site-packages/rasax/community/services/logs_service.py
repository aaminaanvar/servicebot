...
class LogsService(DbService):
    ...
    def fetch_logs(
        ...
        Args:
            ...
            tags_query: Tags separated by `,` whereby the message should match
                all (`AND` filter).
            workspace_id: The integer id of the workspace to retrieve messages from.
                If NONE, returns all items not in a workspace.
        ...
        if workspace_id:
            query = and_(query, MessageLog.workspace_id == workspace_id)
        else:
            query = and_(query, MessageLog.workspace_id.is_(None))
        ...
        total_number_logs = self.cached_query.count(logs)
    ...
    def create_log_from_parse_data(
        ...
        if stored_log:
            if not stored_log.workspace_id:
    ...
    def add_tags_to_message(
        self, log_id: int, tags: List[Dict[Text, Any]]
    ) -> List[Dict[Text, Any]]:
    ...
    def remove_tag_from_message(self, log_id: int, tag_id: int) -> None:
    ...
    def update_logs(
        self, logs_data: List[Dict[Text, Any]]
    ) -> Optional[List[Dict[Text, Any]]]:
        """Updates the existing message logs.

        Args:
            logs_data: Message log information to be updated.
        ...
        for log in logs_data:
            ...
            for field in mutable_fields:
                if field in log:
                    object_kwargs[field] = log.get(field)

            self._merge_log_with_existing(
                MessageLog(**object_kwargs), log_id,
            )

            object_kwargs["id"] = log_id
            updated_logs.append(object_kwargs)
