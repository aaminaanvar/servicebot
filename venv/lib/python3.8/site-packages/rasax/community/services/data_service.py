...
from rasax.community.database.workspace import Workspace
...
class DataService(DbService):
    ...
    def get_training_data_warnings(self, project_id: Text) -> List:
        ...
        ex_per_intent = self.get_intent_example_counts(project_id)
        ...
        for intent_name, examples_for_intent in ex_per_intent.items():
    ...
    def get_intent_example_counts(self, project_id: Text) -> Dict[Text, int]:
        """Gets the number of training examples for each intent."""
        results = (
        ...
        return {name: count for count, name in results}
    ...
    def save_example(
        ...
        if existing_id is not None:
            ...
            new_example_object = self._training_data_object_from_dict(
        ...
        if add_example_items_to_domain:
            self._add_intents_and_entities_to_domain([example], project_id, username)
    ...
    def add_single_example(
        ...
        Returns:
            Data of the created example or None if creation was not successful.
        """
    ...
    def replace_example(
        ...
        updated = self._training_data_object_from_dict(
            example, project_id, filename, user[constants.USERNAME_KEY]
        )
        ...
        self._add_intents_and_entities_to_domain(
            [example], project_id, user[constants.USERNAME_KEY]
        )
    ...
    def _add_intents_and_entities_to_domain(
        self, examples: List[Dict[Text, Any]], project_id: Text, username: Text
    ) -> None:
    ...
    def save_bulk_data(
        ...
        if add_data_items_to_domain:
            self._add_intents_and_entities_to_domain(examples, project_id, username)
    ...
    def _training_data_object_from_dict(
        ...
        rasa.shared.nlu.training_data.util.remove_untrainable_entities_from(data)
    ...
    def create_workspace(self, name: Text) -> Dict[Text, Any]:
        """Create a workspace with a given name.

        Args:
            name: Name to assign to the new workspace.

        Raises:
            ValueError: If the workspace name is already taken.

        Returns:
            New workspace as a dictionary.
        """
        workspace = self.query(Workspace).filter(Workspace.name == name).first()
        if workspace:
    ...
    def get_workspaces(self) -> List[Dict[Text, Any]]:
        """Retrieve a list of existing workspaces.

        Returns:
            List of existing workspaces.
        """
        return [workspace.as_dict() for workspace in self.query(Workspace)]
