...
def dump_responses(responses: List[Response]) -> Dict[Text, Any]:
    """Dump responses in a dictionary/list structure.

    Args:
        responses: List of responses to dump.

    Returns:
        Dictionary containing responses as they would be structured in a domain file.
    """
    response_list = [
        {t.response_name: remove_annotations(json.loads(t.content))} for t in responses
    ]
...
class Domain(Base):
    ...
    to_delete = sa.Column(sa.Boolean, nullable=False, default=False)
    ...
    def as_dict(self) -> Dict[Text, Any]:
        ...
        for s in self.slots:
            ...
            for prop in ["max_value", "min_value", "influence_conversation"]:
                # Only include these properties in the slot dictionary if they
                # have been explicitly set.
                value = getattr(s, prop)
                if value is not None:
                    slots[name][prop] = value

        domain_dict = {
            "config": {"store_entities_as_slots": self.store_entities_as_slots},
            "actions": [e.action for e in self.actions if not e.is_form],
            "forms": {e.action: e.as_dict()["data"] for e in self.actions if e.is_form},
            "entities": [e.entity for e in self.entities],
            "intents": [i.as_dict() for i in self.intents],
            "slots": slots,
            "responses": dump_responses(self.responses),
        }

        if self.filename:
            domain_dict[self.FILENAME_KEY] = self.filename.lstrip("./")
...
class DomainAction(Base):
    ...
    # Starting from Rasa Open Source 2.0, slot mappings for forms are defined
    # in the domain file. To store these mappings, we use the `data` column to
    # store the JSON-serialized information there.
...
class DomainSlot(Base):
    ...
    initial_value = sa.Column(sa.Text)
    type = sa.Column(
        sa.String, default="rasa.shared.core.slots.UnfeaturizedSlot"
    )  # TODO(alwx): add a migration?
    values = sa.Column(sa.Text)
    max_value = sa.Column(sa.Float)
    min_value = sa.Column(sa.Float)
    influence_conversation = sa.Column(sa.Boolean)
