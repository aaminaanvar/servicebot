...
class DataTag(Base):
    ...
    def as_dict(self) -> Dict[Text, Union[Text, int, List[Text]]]:
        return {
            ...
            "value": self.value,
            "color": self.color,
            "conversations": list(self._conversations_set()),
            "message_logs": list(self._logs_set()),
        }

    def _logs_set(self) -> Set[int]:
        return {m.id for m in self.message_logs}

    def _conversations_set(self) -> Set[int]:
        return {c.sender_id for c in self.conversations}
...
class MessageLog(Base):
    ...
    - `workspace_id` (Revision: `45883492f256`)
    ...
    workspace_id = sa.Column(sa.Integer, sa.ForeignKey("workspace.id"))
