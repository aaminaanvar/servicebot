...
class User(Base):
    ...
    default_workspace_id = sa.Column(
        sa.Integer, sa.ForeignKey("workspace.id"), nullable=False, unique=True
    )
    default_workspace = relationship("Workspace", back_populates="username")
    ...
    def as_dict(self, return_api_token=False) -> "UserDict":
        """Return a JSON-like representation of this `User`.

        Args:
            return_api_token: If `True`, include API token in return value.

        Returns:
            Dictionary containining user attributes.
        """
