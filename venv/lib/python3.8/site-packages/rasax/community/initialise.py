...
class AppStartedCallable:
    ...
    def open_web_browser(login_url: Text) -> None:
        """Opens a new tab on the user's preferred web browser and points it to
        `login_url`.
    ...
    def __call__(self) -> None:
        """Execute a set of actions that should be executed after the successful
        application start.
        In local mode, this callable prints a login url to console and opens a
        browser window.
        Otherwise, it checks if the password was generated, and then just prints this
        password if it was.
        """
...
def create_default_workspaces(session: orm.Session) -> None:
    """Ensure all Rasa X users in the DB have a default workspace assigned to them.
    ...
    users = user_service.fetch_all_users(rasa_x_config.team_name)

    for user in users:
        if not user["default_workspace_id"]:
            name = constants.WORKSPACE_NAME_TEMPLATE.format(user["username"])
            workspace = data_service.create_workspace(name)
            user_service.update_user(
                user["username"], {"default_workspace_id": workspace["id"], "data": ""}
            )
